...
import { Upload, FileText, CheckCircle, Loader, MessageSquare, Send, LogOut } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const [chatHistory, setChatHistory] = useState([
        { role: 'system', content: 'Hello! Upload a PDF to get started, or ask me anything about your uploaded documents.' },
    ]);
    ...
    const handleQuery = async (e) => {
        ...
        try {
            ...
            setChatHistory(prev => [...prev, { role: 'bot', content: accumulatedText }]);
            ...
            <header className="flex items-center justify-between w-full bg-white/5 border border-white/10 rounded-2xl p-4 px-6 backdrop-blur-md shadow-lg">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-400 font-medium">Enterprise SOP Intelligence</p>
                    </div>
                </div>
                ...
                >
                    ...
                    <div className="border-2 border-dashed border-gray-600 rounded-xl p-8 flex flex-col items-center justify-center gap-4 hover:border-blue-500 transition-colors bg-black/20 group">
                        ...
                        <div className="text-center">
                            <p className="text-gray-300 font-medium mb-1">Drag & Drop your PDF</p>
                    ...
                    {file && (
                        <div className="flex items-center gap-3 bg-blue-500/10 border border-blue-500/20 p-3 rounded-xl">
                            ...
                            <span className="truncate text-sm font-medium text-blue-100">{file.name}</span>
                        </div>
                    )}
                    ...
                    >
                        {uploading ? <Loader className="animate-spin" /> : <CheckCircle size={20} />}
                        {uploading ? "Ingesting Document..." : "Process Document"}
                    </button>

                    {uploadStatus && (
                        <p className={`text-center text-xs font-bold uppercase tracking-wide ${uploadStatus.includes('Success') ? 'text-green-400' : 'text-red-400'}`}>
                            {uploadStatus}
                        </p>
                    )}
                </motion.div>
